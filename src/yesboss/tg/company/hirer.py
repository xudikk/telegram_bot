...
from .vacancy import insertVacancies, get_file_path, change_position, change_age, change_languages, change_media, \
    change_schedule, change_vacancy_gender, change_experience, change_salary, change_region, change_district, \
    delete_vacancy
from .hirer_trans import Texts
from .hirer_service import *
...
class Hirer(UserData):
    ...
    def get_main_buttons(self, back_btn=False, type_btn=None):
        ...
        elif type_btn == 'change_langs':
            buttons.append([Texts['BTN_SELECT_LANGUAGE_UZ'][lang]])
            buttons.append([Texts['BTN_SELECT_LANGUAGE_RU'][lang]])
            buttons.append([Texts['BTN_BACK'][lang]])
        else:
            buttons.append([Texts['BTN_ADD_VACANT'][lang], Texts['BTN_FIND_RESUME'][lang]])
            buttons.append([Texts['BTN_REPLY'][lang], Texts['BTN_VACANTS'][lang]])
    ...
    def get_search_buttons(self):
        ...
        lang = self.user_model['lang']
        buttons = [[Texts['SEARCH_CATEGORY'][lang]], [Global['BTN_HOME'][lang]]]
            ...
            if category['id'] not in cats:
                inline_keyboard.append(
                    [InlineKeyboardButton(category[f'name_{lang}'],
                                          callback_data=f'add_vacancy_categorychild_{category["id"]}')])
                ...
                inline_keyboard.append(
                    [
                        InlineKeyboardButton(Texts['BTN_SEND_BACK'][lang],
                                             callback_data=f'add_vacancy_categorychild_back'),
                        InlineKeyboardButton(Texts['BTN_SEND_DALE'][lang],
                                             callback_data=f'add_vacancy_categorychild_next')
                    ])
            ...
            if len(inline_keyboard) > 0:
                if len(langs) > 0:
                    inline_keyboard.append(
                        [
                            InlineKeyboardButton(Texts['BTN_SEND_DALE'][lang],
                                                 callback_data=f'add_vacancy_language_next')
                        ])
                if message_id:
    ...
    def send_vacancy_publication(self, vacancy, number, cnt):
        try:
            lang = self.user_model['lang']
            inline_keyboard = [
                [
                    InlineKeyboardButton(self.send_trans("BTN_EDIT"),
                                         callback_data=f'vacancy_edit_show_{vacancy["vacancy_id"]}_{number}_{cnt}'),
                    InlineKeyboardButton(self.send_trans("BTN_DELETE"),
                                         callback_data=f'vacancy_delete_show_{vacancy["vacancy_id"]}_{number}_{cnt}')
                ]
            ]
            if int(number) == 1 and int(number) == int(cnt):
                inline_keyboard.append([
                    InlineKeyboardButton(f'{number}/{cnt}', callback_data=f'vacancy_not')]
                )
            elif int(number) == 1:
                inline_keyboard.append([
                    InlineKeyboardButton(f'{number}/{cnt}', callback_data=f'vacancy_not'),
                    InlineKeyboardButton('▶️',
                                         callback_data=f'vacancy_next_{vacancy["vacancy_id"]}_{number}_{cnt}')
                ])
            elif int(number) == int(cnt):
                inline_keyboard.append([
                    InlineKeyboardButton('◀️',
                                         callback_data=f'vacancy_prev_{vacancy["vacancy_id"]}_{number}_{cnt}'),
                    InlineKeyboardButton(f'{number}/{cnt}', callback_data=f'vacancy_not')]
                )
    ...
    def all_preview(self, resume):
        try:
            lang = self.user_model['lang']
            fio = f"{resume['firstname']} {resume['lastname']} {resume['middlename']}"
            phone_number = resume["phone_number"]
            post = f"💼 {fio}️"
            post = "{}\n{} - {}".format(post, Texts['POSITION'][lang], resume["position_name"])
            ...
            post = f"{post}\n{Texts['AGE'][lang]} - {resume['user_age']}️"
    ...
    def send_all_publication(self, resume, number, cnt, keyword):
        ...
        if int(number) == 1 and int(number) == int(cnt):
            ...
            inline_keyboard.append([
                ...
                InlineKeyboardButton('▶️',
                                     callback_data=f'search_next_{resume["resume_id"]}_{number}_{cnt}_{keyword}')
            ])
        elif int(number) == int(cnt):
            inline_keyboard.append([
                InlineKeyboardButton('◀️',
                                     callback_data=f'search_prev_{resume["resume_id"]}_{number}_{cnt}_{keyword}'),
                InlineKeyboardButton(f'{number}/{cnt}', callback_data=f'search_not')]
            )
        ...
        inline_keyboard.append([
            InlineKeyboardButton('Номер телефона',
                                 callback_data=f'contact_resume_{resume["resume_id"]}')
        ])
        if file_type == 1:
            file_type = 'photo'
        else:
    ...
    def send_edit_vacancy_inline(self, vacancy_id, number, cnt, type_btn=None, region_id=None):
        lang = self.user_model['lang']
        ...
        if type_btn == None:
            inline_keyboard = [
                [
                    InlineKeyboardButton(self.send_trans('POSITION'),
                                         callback_data=f'vacancy_edit_position_{vacancy_id}_{number}_{cnt}'),
                    InlineKeyboardButton(self.send_trans('SALARY'), callback_data=f'vacancy_edit_salary_{vacancy_id}_{number}_{cnt}'),
                ],
                [
                    InlineKeyboardButton(self.send_trans('SCHEDULE'),
                                         callback_data=f'vacancy_edit_schedule_select_{vacancy_id}_{number}_{cnt}'),
                    InlineKeyboardButton(self.send_trans('LANGS'),
                                         callback_data=f'vacancy_edit_langs_select_{vacancy_id}_{number}_{cnt}'),
                ],
                [
                    InlineKeyboardButton(self.send_trans('AGE'), callback_data=f'vacancy_edit_age_{vacancy_id}_{number}_{cnt}'),
                    InlineKeyboardButton(self.send_trans('EXPERIENCE'),
                                         callback_data=f'vacancy_edit_experience_select_{vacancy_id}_{number}_{cnt}'),
                ],
                [
                    InlineKeyboardButton(self.send_trans('GENDER'),
                                         callback_data=f'vacancy_edit_genders_select_{vacancy_id}_{number}_{cnt}'),

                    InlineKeyboardButton(self.send_trans('REGION'),
                                         callback_data=f'vacancy_edit_region_select_{vacancy_id}_{number}_{cnt}'),
                ],
                [
                    InlineKeyboardButton(self.send_trans('EDITED_MEDIA'),
                                         callback_data=f'vacancy_edit_media_{vacancy_id}_{number}_{cnt}'),
                ]
            ]
        elif type_btn == 'salary':
            inline_keyboard = [
                [
                    InlineKeyboardButton(self.send_trans('BTN_SALARY_YES'),
                                         callback_data=f'vacancy_edit_salary_yes_{vacancy_id}_{number}_{cnt}'),
                    InlineKeyboardButton(self.send_trans('BTN_SALARY_NO'),
                                         callback_data=f'vacancy_edit_salary_no_{vacancy_id}_{number}_{cnt}')
                ]
            ]
                ...
                inline_keyboard.append(
                    [
                        InlineKeyboardButton(schedule[f'name_{lang}'],
                                             callback_data=f'vacancy_edit_schedule_{schedule["id"]}_{vacancy_id}_{number}_{cnt}')
                    ]
                )
        elif type_btn == 'langs':
            languages = services.getLanguages()
            for language in languages:
                inline_keyboard.append(
                    [
                        InlineKeyboardButton(language[f'name_{lang}'],
                                             callback_data=f'vacancy_edit_langs_{language["id"]}_{vacancy_id}_{number}_{cnt}')
                    ]
                )
            ...
            for gender in genders:
                inline_keyboard.append(
                    [InlineKeyboardButton(gender['name'],
                                          callback_data=f'vacancy_edit_genders_{gender["id"]}_{vacancy_id}_{number}_{cnt}')])
                ...
                inline_keyboard.append(
                    [
                        InlineKeyboardButton(experience[f'name_{lang}'],
                                             callback_data=f'vacancy_edit_experience_{experience["id"]}_{vacancy_id}_{number}_{cnt}')
                    ]
                )
                ...
                inline_keyboard.append(
                    [
                        InlineKeyboardButton(region[f'name_{lang}'],
                                             callback_data=f'vacancy_edit_region_{region["id"]}_{vacancy_id}_{number}_{cnt}')
                    ]
                )
                ...
                inline_keyboard.append(
                    [
                        InlineKeyboardButton(district[f'name_{lang}'],
                                             callback_data=f'vacancy_edit_region_district_{district["id"]}_{vacancy_id}_{number}_{cnt}')
                    ]
                )

        inline_keyboard.append(
            [
                InlineKeyboardButton(Texts['BTN_BACK'][lang], callback_data=f'vacancy_edit_back_{vacancy_id}_{number}_{cnt}')
            ]
        )
        ...
        if not company_model:
            ...
            if not company_model.is_terms:
                ...
                if msg == self.get_trans_global('BTN_HOME'):
                    ...
                    user_id = self.user_model['id']
                    vac, cnt = getMyVacancies(user_id, 1), get_cnt_vac(self.user.id)
                    self.send_vacancy_publication(vac, cnt=cnt["cnt"], number=1)

                elif msg == self.get_trans('BTN_FIND_RESUME'):
                    self.change_state({"search_state": 1, 'state': 200, 'search_text': txt})
                    ...
                    resume, cnt = self.get_find_resume(txt), self.get_cnt_res(txt)
                    print("resume: ", resume, "\n", "cnt: ", cnt)
                    self.send_all_publication(resume=resume, number=1, cnt=cnt["cnt"], keyword=txt)
                elif search_state == 2:
                    resume, cnt = self.get_find_resume(txt), self.get_cnt_res(txt)
                    print("resume: ", resume, "\n", "cnt: ", cnt)
                    self.send_all_publication(resume=resume, number=1, cnt=cnt["cnt"], keyword=txt)
                ...
                # SETTINGS
                    ...
                    self.change_state({'state': 401, 'company_name': txt})
                    self.go_message(self.user.id, self.send_trans('TEXT_START_NAME'),
                                    self.get_main_buttons(type_btn='back'))
                elif msg == self.get_trans('TEXT_PHONE_COMPANY'):

                    self.change_state({'state': 402, 'data': txt})
                    self.go_message(self.user.id, self.send_trans('TEXT_START_CONTACT'), ReplyKeyboardMarkup(
                        [[KeyboardButton(self.send_trans('BTN_SEND_PHONE'), request_contact=True)]],
                        one_time_keyboard=True,
                        resize_keyboard=True))
                    ...
                    self.go_message(self.user.id, Global['BTN_SELECT_LANG'][lang],
                                    self.get_main_buttons(type_btn='change_langs'))

                # END SETTING

                elif msg == self.get_trans_global('BTN_SEND_BACK'):
                    print('come BTN_SEND_BACK')
                    if user_state == 8:
                        self.change_state({'state': 10})
                        self.go_message(self.user.id, self.send_trans('TEXT_HOMEPAGE'), self.get_main_buttons())
                    elif user_state == 55:
                ...
                # END SCHEDULE BACK
                    ...
                    if user_state == 12:
                        ...
                        self.go_message(self.user.id, Texts['TEXT_SEND_MEDIA'][lang],
                                        self.get_main_buttons(back_btn=True))
                    elif user_state == 14:
                ...
                #   END SETTINGS BACK BUTTONS
                    ...
                    if user_state == 8:
                        ...
                        print('send_schedules')
                        self.change_state({'state': 57, 'salary_type': txt})
                        self.send_schedules()
                    elif user_state == 57:
                        sche = self.get_schedule(txt)
                        self.change_state({'state': 58, "schedule_id": sche['id'], 'langs': []})
                        langs = self.getLanguages([])
                        ...
                        if msg == self.get_trans_global('BTN_SEND_DALE'):
                            self.change_state({'state': 59})
                            self.send_experience()
                        else:
                    ...
                    #         edited
                        ...
                        self.change_state({'state': 100})
                        vacancy_id = self.user_data.get('vacancy_id', 0)
                        change_position(vacancy_id, txt)
                        vac = get_vacancies_one(vacancy_id)
                        self.send_vacancy_publication(vac, number=number, cnt=cnt)
                    elif user_state == 105:
                        self.change_state({'state': 100})
                        vacancy_id = self.user_data.get('vacancy_id', 0)
                        ...
                        try:
                            data = getResumeByName(txt)

                        except Exception as e:
                            print('errror:  ', e)

                    elif user_state == 401:
                        company_model.company_name = txt
                        company_model.save()
                        self.change_state({'state': 400})
                        self.go_message(self.user.id, Texts['TEXT_SETTINGS'][lang],
                                        self.get_main_buttons(type_btn='settings'))
                    elif user_state == 402:
                        company_model.phone_number = txt
                        company_model.save()
                        self.change_state({'state': 400})
                        self.go_message(self.user.id, Texts['TEXT_SETTINGS'][lang],
                                        self.get_main_buttons(type_btn='settings'))
                    elif user_state == 403:
                        print('self.user_model: ', self.user_model)
    ...
    def inline_query(self, data_sp, message_id, chat_id, query_id):
        ...
        if data_sp[0] == 'add':
            if data_sp[2] == 'category':
                ...
                if data_sp[3] == 'yes':
                    ...
                    self.clear_state(10)
                    self.delete_message_user(chat_id, message_id)
                    self.go_message(self.user.id, self.send_trans('MAIN_MENU'), self.get_main_buttons())
        # Xudoyberdi search section

        elif data_sp[0] == "search":
            if data_sp[1] == "next":
                resume = self.get_find_resume(keyword=data_sp[5], count=int(data_sp[3]) + 1)
                self.send_all_publication(resume=resume, number=int(data_sp[3]) + 1, cnt=int(data_sp[4]),
                                          keyword=data_sp[5])
                self.delete_message_user(chat_id, message_id)
            elif data_sp[1] == "prev":
                resume = self.get_find_resume(keyword=data_sp[5], count=int(data_sp[3]) - 1)
            ...
            if data_sp[1] == "resume":
                ...
                print(phone_number)
                self.answer_callback_query(query_id,
                                               f'{phone_number["comp_name"]} \n{phone_number["phone_number"]}')
        elif data_sp[0] == "contact":
            phone_number = self.get_resume_phone(data_sp[2])
        ...
        # edited
            ...
            user_id = self.user_model['id']
            if data_sp[1] == "next":
                vacancy, cnt = getMyVacancies(user_id, int(data_sp[3]) + 1), get_cnt_vac(self.user.id)
                self.send_vacancy_publication(vacancy=vacancy, number=int(data_sp[3]) + 1, cnt=cnt["cnt"])
                ...
                if data_sp[2] == 'show':
                ...
                elif data_sp[2] == 'position':
                    self.change_state({'state': 101, 'vacancy_id': data_sp[3], "number": data_sp[-2],
                                           "cnt": data_sp[-1]})
                    ...
                    if data_sp[3] == 'yes':
                        ...
                        vac = get_vacancies_one(vacancy_id)
                        self.send_vacancy_publication(vac, number=data_sp[-2], cnt=data_sp[-1])
                    else:
                        self._bot.edit_message_reply_markup(
                            chat_id=chat_id, message_id=message_id,
                            reply_markup=InlineKeyboardMarkup(self.send_edit_vacancy_inline(vacancy_id=data_sp[3],
                    ...
                    if data_sp[3] == 'save':
                        ...
                        for language in languages:
                            if language['id'] not in langs:
                                inline_keyboard.append(
                                    [InlineKeyboardButton(language[f'name_{lang}'],
                                                          callback_data=f'vacancy_edit_langs_{language["id"]}_{data_sp[4]}_{data_sp[-2]}_{data_sp[-1]}')])

                        inline_keyboard.append(
                            [
                                InlineKeyboardButton("Yakunlash", callback_data=f'vacancy_edit_langs_save_{data_sp[4]}_{data_sp[-2]}_{data_sp[-1]}')
                            ])
                        self._bot.edit_message_reply_markup(
                            chat_id=chat_id, message_id=message_id,
                            reply_markup=InlineKeyboardMarkup(inline_keyboard)
                    ...
                    self.change_state({'state': 105, 'vacancy_id': data_sp[3], "number": data_sp[-2],
                                           "cnt": data_sp[-1]})
                    self.go_message(self.user.id, self.send_trans('TEXT_SEND_AGE'), self.get_main_buttons(True))
                    self.delete_message_user(chat_id, message_id)
                            ...
                            if gender['id'] not in genders:
                                inline_keyboard.append(
                                    [InlineKeyboardButton(gender[f'name'],
                                                          callback_data=f'vacancy_edit_genders_{gender["id"]}_{data_sp[4]}_{data_sp[-2]}_{data_sp[-1]}')])

                        inline_keyboard.append(
                            [
                                InlineKeyboardButton("Yakunlash",
                                                     callback_data=f'vacancy_edit_genders_save_{data_sp[4]}_{data_sp[-2]}_{data_sp[-1]}')
                            ])
                    ...
                    self.change_state({'state': 109, 'vacancy_id': data_sp[3], "number": data_sp[-2],
                                           "cnt": data_sp[-1]})
                    self.delete_message_user(chat_id=chat_id, message_id=message_id)
                    self.go_message(chat_id, Texts['TEXT_SEND_MEDIA'][lang], self.get_main_buttons(True))

                elif data_sp[2] == 'back':
                    self.change_state({"number": data_sp[-2], "cnt": data_sp[-1]})
                    self.delete_message_user(chat_id, message_id)
    ...
    def get_media_photo(self, file_id, file_type, ):
        ...
        print('media user_state: ', user_state)
